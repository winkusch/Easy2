...
EASYMERGE.read <- function(object) {
	...
	if(!object@blnUseFastRead) {
		...
		object@tblGWA 	<- tryCatch(
			...
			#fread(strFread, header=TRUE, na.strings = object@astrMissing, stringsAsFactors=FALSE, colClasses = object@aClassesRead, skip=object@numRowSkip, select = aSelect, tmpdir=object@pathTmp),
			error = function(err) {
				strError = err$message
				val=strsplit(strError,"'",fixed=T)[[1]][length(strsplit(strError,"'",fixed=T)[[1]])]
				g=scan(file = object@fileIn, what=character(0), n = -1,sep = "\n",quiet=TRUE)
...
EASYMERGE.run <- function(objEASYMERGE, objGWA) {
	...
	strSuffixIn = objGWA@strSuffix
	if(objGWA@numMergeIdx>0) strSuffixIn = ""
	objGWA@numMergeIdx = objGWA@numMergeIdx + 1
	
	strSuffixAdd = objEASYMERGE@strSuffix
	if(nchar(strSuffixIn)>0 & substr(strSuffixIn,1,1)!=".") strSuffixIn = paste0(".",strSuffixIn)
	if(nchar(strSuffixAdd)>0 & substr(strSuffixAdd,1,1)!=".") strSuffixAdd = paste0(".",strSuffixAdd)
	
	colMergeIn = objGWA@colMerge
	if(colMergeIn=="") colMergeIn = objEASYMERGE@colMerge
	colMergeAdd = objEASYMERGE@colMerge
	
	objGWA.merged <- GWADATA.merge(objGWA, objEASYMERGE, 
						strSuffix.In = strSuffixIn, 
						strSuffix.Add = strSuffixAdd, 
						blnAll.In = TRUE, 
						blnAll.Add = FALSE, 
						strBy.In = colMergeIn, 
						strBy.Add = colMergeAdd
					) 
