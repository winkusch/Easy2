...
GETVAR.run <- function(objGV, objGWA, objREPORT) {
	...
	# objGWA = GWADATA.cbind(objGWA, NA, "GETVAR.alleleadjusted") {
	
	if(blnAdjustAlleles2Ref) objGWA = GWADATA.cbind(objGWA, NA, "GETVAR.alleleadjusted")
	...
	if(nrow(objGWA.var@tblGWA)==1 & blnAdjustAlleles2Ref) {
		if(toupper(strAlleleRef) == toupper(objGWA.var@tblGWA[[colInA1]])) {
			## all ok
			objGWA.var = GWADATA.cbind(objGWA.var, TRUE, "GETVAR.alleleadjusted", blnOverwrite=TRUE)			
			...
			objGWA.var = GWADATA.cbind(objGWA.var, FALSE, "GETVAR.alleleadjusted", blnOverwrite=TRUE)
		}
	} 
