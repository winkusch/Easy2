...
GWADATA.define <- function(objGWA, strConfigCommand, pathOut=NA) {
	...
	## this overrides pathOut by the "passed" pathOut
	if(!is.na(pathOut)) objGWA@pathOut = pathOut
...
GWADATA.init <- function(object) {
	...
	####################################################################
	##### Check availablity of tmp dir
	if(object@pathTmp!="" & !file.exists(object@pathTmp)) {
		stop(paste("EASY ERROR:\n Temporary path \n",object@pathTmp,"\n does not exist!!!\n", sep=""))
	}
...
GWADATA.read <- function(object) {
	...
	if(!object@blnUseFastRead) {
		...
		# if(isGz | isBgz) {
			# strFread <- paste("zcat ",object@fileIn,sep="")
		# } else {
			# strFread <- object@fileIn
		# }
		...
		object@tblGWA 	<- tryCatch(
			#fread(strFread, header=object@blnHeader, na.strings = object@astrMissing, sep = object@strSeparator, stringsAsFactors=FALSE, colClasses = object@aClassesRead, skip=object@numRowSkip, select = aSelect),
			fread(object@fileIn, header=object@blnHeader, na.strings = object@astrMissing, sep = object@strSeparator, stringsAsFactors=FALSE, colClasses = object@aClassesRead, skip=object@numRowSkip, select = aSelect, tmpdir=ifelse(object@pathTmp=="",tempdir(),object@pathTmp)),
			error = function(err) {
				strError = err$message
				val=strsplit(strError,"'",fixed=T)[[1]][length(strsplit(strError,"'",fixed=T)[[1]])]
				g=scan(file = object@fileIn, what=character(0), n = -1,sep = "\n",quiet=TRUE)
...
GWADATA.merge <- function(objGWA, objGWA.Add, strSuffix.In = ".in", strSuffix.Add = ".add", blnAll.In = FALSE, blnAll.Add = FALSE, strBy.In = NA, strBy.Add = NA) {
	...
	## objGWA,objGWA.indep, strSuffix.In = "", strSuffix.Add = "", blnAll.In = TRUE, blnAll.Add = TRUE, strBy.In = NA, strBy.Add = NA
	# objGWA.Add = objGWA.indep
	# strSuffix.In = ""
	# strSuffix.Add = ""
	# blnAll.In = TRUE
	# blnAll.Add = TRUE
	...
	# xt1 = objGWA@tblGWA
	# xt2 = objGWA.Add@tblGWA
	# xby = strBy.In
	# xallx = blnAll.In
	# xally = blnAll.Add
	# xsuff = c(strSuffix.In, strSuffix.Add)
	
	# to = merge(xt1, xt2, by = xby, all.x = xallx, all.y = xally, suffixes = xsuff,allow.cartesian=TRUE)
	
	objGWA@tblGWA <- as.data.table(merge(as.data.frame(objGWA@tblGWA), as.data.frame(objGWA.Add@tblGWA), 
								by = strBy.In, 
...
GWADATA.write <- function(objGWA, strMode = "txt", strPrefix = "", strSuffix = "", strSep = "\t", strMissing = "NA", strTabixParam="", blnHeader = TRUE, blnWrite10rows=FALSE, colSnps = "rsid", blnWriteSnpList=FALSE, strSnpListSep = "\n") {		
	...
	blnUseFastWrite <- objGWA@blnUseFastWrite
	
	if(objGWA@strRetainHead != "" | objGWA@strRetainEnd != "") blnUseFastWrite = FALSE
	...
	if(strMode == "gz") {
		...
		if(blnUseFastWrite) {
			...
			fwrite(objGWA@tblGWA, fileOut, compress=c("gzip"), row.names=F, quote=F, sep=strSep, na=strMissing, col.names=blnHeader)	
		...
		if(blnUseFastWrite) {
			fwrite(objGWA@tblGWA, fileOut, row.names=F, quote=F, sep=strSep, na=strMissing, col.names=blnHeader)		
