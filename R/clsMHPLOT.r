setClass("MHPLOT",
	representation = representation(
						...
						strColourNovel			=	"character",
						rcdAnnotSignif			=	"character",
						blnPlotVerticalLines	=	"logical"
						),
	prototype = prototype(
						strEqcCommand			=	"",
						colMHPlot				=	"",
...
setMethod("setMHPLOT", signature = (object = "MHPLOT"), function(object) {
	...
	aEqcSlotNamesIn = c("colMHPlot","colInChr","colInPos",
						...
						"fileAnnot","numAnnotPosLim","numAnnotPvalLim","strColourNovel","rcdAnnotSignif","blnPlotVerticalLines","numYAxisLim")
...
MHPLOT.GWADATA.valid <- function(objMHPLOT, objGWA) {
	...
	if(!(objMHPLOT@strFormat %in% c("png","pdf","tiff","jpeg")))
	...
	if(objMHPLOT@fileAnnot != "") {
		if(!file.exists(objMHPLOT@fileAnnot))
			stop(paste("EASY ERROR:MHPLOT\n File fileAnnot\n ",objMHPLOT@fileAnnot,"\n does not exist.", sep=""))
		### Cols exist?
			tblAnnot10<-read.table(objMHPLOT@fileAnnot,header=T, sep="\t", stringsAsFactors=FALSE, nrows = 10,comment.char="",quote="")
			...
			if(objMHPLOT@strColourNovel == "") {
				isAv <- "Colour" %in% names(tblAnnot10) | "Exclude" %in% names(tblAnnot10)
				if(!isAv)
					stop(paste(" EASY ERROR:MHPLOT\n Column 'Colour' or 'Exclude' are not available in fileAnnot. PLease correct file header OR set --strColourNovel to some colour.", sep=""))	
			}
	}
	
	
	if(objMHPLOT@numYAxisLim!=Inf & objMHPLOT@blnYAxisBreak) stop(paste(" EASY ERROR:MHPLOT\n You cannot set --numYAxisLim with --blnYAxisBreak 1. PLease remove one parameter.", sep=""))	
...
MHPLOT.run <- function(objMHPLOT, objGWA) {
	...
	strColourNovel		=	objMHPLOT@strColourNovel
	rcdAnnotSignif			=	objMHPLOT@rcdAnnotSignif
	blnPlotVerticalLines	=	objMHPLOT@blnPlotVerticalLines
	
	numYAxisLim				=	objMHPLOT@numYAxisLim
	...
	signifplot <- rep(FALSE, dim(objGWA@tblGWA)[1])
	
	if(rcdAnnotSignif!="") {
		objRCD 	<- RCD(rcdAnnotSignif)
	...
	if(blnLogPval) {
		...
		if(any(isRemove)) {
			...
			signifplot <- signifplot[-which(isRemove)]
		}
	} else {
	...
	tblPlot <- as.data.frame(objGWA@tblGWA, stringsAsFactors=FALSE)
	
	tblPlot <- setNames(tblPlot, c("y","chr","pos"))
	
	#names(tblPlot)[1] = "y"
	#names(tblPlot)[2] = "chr"
	#names(tblPlot)[3] = "pos"
	
	tblPlot = cbind(tblPlot, colplot, plotCritKey, symplot, cexplot, signifplot, stringsAsFactors = FALSE)
	...
	if(fileAnnot != "") {
		...
		if(rcdAnnotSignif=="") {
			...
			isSignif <- tblPlot$signifplot
		}
		
		isUseExclude = any(names(tblAnnot)=="Exclude")
		isExclude = rep(FALSE,nrow(tblPlot))
		
		isUseColour = any(names(tblAnnot)=="Colour")
		isUseColourOrder = any(names(tblAnnot)=="ColourOrder")
		
		isUseColourNovel = strColourNovel!=""
		isKnown = rep(FALSE,nrow(tblPlot))
		
		maxCritKey = max(tblPlot$plotCritKey, na.rm=T)
		...
		for(iLocus in 1:nrow(tblAnnot)) {
			...
			if(any(isOverlap)) {
				...
				if(isUseColour) {
					...
					if(isUseColourOrder) {
						tblPlot$plotCritKey[isLocus] <- maxCritKey + tblAnnot$ColourOrder[iLocus]
					} else {
				...
				if(isUseExclude) {
					isLocusExclude = tblAnnot$Exclude[iLocus]==1
					isLocusExclude[is.na(isLocusExclude)] = FALSE
					isExclude[isLocus] = isLocusExclude
				}
				
				if(isUseColourNovel) {
					isKnown[isLocus] = TRUE
				}
			}
		}
		if(isUseExclude) {
			tblPlot = tblPlot[!isExclude,]
			warning(paste("EASY WARNING:\n There are ",length(which(isExclude)), " SNPs being excluded from the MH plot due to Exclude column in \n ", fileAnnot ,sep="" ))
		}
		if(isUseColourNovel) {
			isNovel = isSignif & (!isKnown)
			tblNovel = tblPlot[isNovel,]
			for(iVar in 1:nrow(tblNovel)) {
				isNovelLocus = (tblPlot$chr == tblNovel$chr[iVar]) & (abs(tblPlot$pos-tblNovel$pos[iVar])<numAnnotPosLim)
				tblPlot$colplot[isNovelLocus] <- strColourNovel
				tblPlot$plotCritKey[isNovelLocus] <- Inf
			}
		}
	}
	...
	if(numYAxisLim == Inf) numYAxisLim = max(tblPlot$y)
	
	plot( tblPlot$x, tblPlot$y, col=tblPlot$colplot, pch=tblPlot$symplot, cex=tblPlot$cexplot, cex.lab = numCexLab, xaxt="n",yaxt="n", ylab=strYlab, xlab="",ylim=c(0,numYAxisLim))
	
	# Achsenlinien:
	axis(2, at =yAxisTickPos,  labels = yAxisTickLab, cex.axis = numCexAxis)
	mtext("Chromosome", side=1, line=2.5, cex=numCexLab, padj=2)
	
	# axis(1,at=arr_labpos,labels=arr_labname,las=3,cex.axis = 0.8*numCexAxis)
	mapply(axis, side = 1, at = arr_labpos, labels = arr_labname, las=3,cex.axis = 0.8*numCexAxis)
	
	abline(a=0,b=0,col="gray3",lty=6)
	if(blnPlotVerticalLines) {
