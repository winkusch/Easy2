...
setMethod("setGC", signature = (object = "GC"), function(object) {
	...
	aEqcSlotNamesIn = c("colPval", "colSE", "numLambda" , "numChiSqDf", "fileGcSnps", "colGcSnpsMarker", "colInMarker", "blnSuppressCorrection","strTag","rcdGcCrit")
...
GC.run <- function(objGC, objGWA, objREPORT) {
	...
	aPvalNoGc <- aPvalNoGcCalc <- GWADATA.getcol(objGWA, colPval)
	aChisqNoGc<- aChisqNoGcCalc<- qchisq(aPvalNoGc,df=numChiSqDf,lower.tail=F)
	
	if(objGC@numLambda == -1) {
		## Calculate Lambda
		isUsedForGc = rep(TRUE,length(aPvalNoGc))
		
		if(objGC@fileGcSnps != "") {
			# If file is defined, use SNPs in file for calculation of Lambda
			aMarker <- GWADATA.getcol(objGWA, objGC@colInMarker)
		...
		} else if(objGC@rcdGcCrit != "") {		
			
			objRCD 		<- RCD(objGC@rcdGcCrit)
			isUsedForGc <- RCD.eval(objRCD, objGWA)
		}
		...
		denom<-qchisq(0.5, df=numChiSqDf)
		objGC@numLambda<-median(aChisqNoGcCalc,na.rm=T)/denom
		
	}
	
	
	strNewColName <- paste(colPval,strTag,sep=".")
	...
	if(!objGC@blnSuppressCorrection) {
		...
		if(objGC@numLambda > 1 & !is.na(objGC@numLambda)) {
			...
			aPval1Gc<-signif(pchisq(aChisq1Gc,df=numChiSqDf,lower.tail=F),4)
		} else {
			aPval1Gc <- signif(pchisq(aChisqNoGc,df=numChiSqDf,lower.tail=F),4)
		}
		...
		if(!is.na(iSe)) {
			...
			strNewColNameSe = sub(colPval,colSE,strNewColName)
			objGWA <- GWADATA.cbind(objGWA, aSe1Gc, strNewColNameSe)
	...
	objREPORT <- REPORT.addval(objREPORT,paste("Lambda.",strNewColName,sep=""), objGC@numLambda)
