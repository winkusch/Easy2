...
validRPLOT <- function(objRPLOT) {
	...
	if(objRPLOT@numDefaultLwd <= 0)
		stop(paste("EASY ERROR:SPLOT\n Wrong numDefaultLwd defined. PLease use positive numeric value.", sep=""))
	...
	if(objRPLOT@anumLwd[1] != -1) {
		for(i in 1:length(objRPLOT@anumLwd)) {
			numLwd = objRPLOT@anumLwd[i]
			if(numLwd <= 0)
				stop(paste("EASY ERROR:SPLOT\n Wrong anumLwd defined. PLease use positive numeric value.", sep=""))			
		}
	}
	
	if(!(objRPLOT@strFormat %in% c("png","pdf","tiff","jpeg")))
...
RPLOT.REPORT.valid <- function(objRPLOT, objREPORT) {
	...
	if(objRPLOT@blnPlotCI) {
		objRCD 	<- RCD(objRPLOT@rcdCIlengthX)
		cix		<- RCD.eval.report(objRCD, objREPORT)
		objRCD 	<- RCD(objRPLOT@rcdCIlengthY)
		ciy 	<- RCD.eval.report(objRCD, objREPORT)
	}
...
RPLOT.run <- function(objRPLOT, objREPORT) {
	...
	numDefaultLwd			=	objRPLOT@numDefaultLwd
	...
	arcdLwdCrit				=	objRPLOT@arcdLwdCrit
	anumLwd					=	objRPLOT@anumLwd
	...
	blnPlotCI 				=	objRPLOT@blnPlotCI
	rcdCIlengthX 			=	objRPLOT@rcdCIlengthX
	rcdCIlengthY 			=	objRPLOT@rcdCIlengthY
	strCIColour				=	objRPLOT@strCIColour
	
	blnAxesLines 			=	objRPLOT@blnAxesLines
	...
	plot.lwd 	= rep(numDefaultLwd, dim(tblPlot)[1])
	...
	if(arcdLwdCrit[1] != "") {
		for(iLwdCrit in 1:length(arcdLwdCrit)) {
			
			rcdLwdCrit 		<- arcdLwdCrit[iLwdCrit]
			numLwd 			<- anumLwd[iLwdCrit]
		
			objRCD 	<- RCD(rcdLwdCrit)
			out 	<- RCD.eval.report(objRCD, objREPORT)
	...
	if(blnPlotCI) {
		...
		ci_x 		<- RCD.eval.report(objRCD, objREPORT)
		objRCD 		<- RCD(rcdCIlengthY)
		ci_y 		<- RCD.eval.report(objRCD, objREPORT)
	}
	...
	if(rcdExclude!="") {
		...
		if(any(out)) {
			...
			plot.cex 			= plot.cex[which(!out)]
			plot.lwd 			= plot.lwd[which(!out)]
	...
	plot.cex = plot.cex[which(!ablnIsNA)]
	plot.lwd 			= plot.lwd[which(!ablnIsNA)]
	...
	plot.cex 	= plot.cex[which(!isExclude)]
	plot.lwd 			= plot.lwd[which(!isExclude)]
	...
	plot.cex 	= plot.cex[iSort]
	plot.lwd 	= plot.lwd[iSort]
	...
	if(strAxes == "equal")  {
		...
		plot.xlims[1] <- plot.ylims[1] <- min(x1tmp, y1tmp)
		plot.xlims[2] <- plot.ylims[2] <- max(x2tmp, y2tmp)
	...
	if(blnPlotCI) {
		...
		# overwrite arows:
		points( x, y, col=plot.colour,pch=plot.symbol,cex=plot.cex, lwd=plot.lwd)
