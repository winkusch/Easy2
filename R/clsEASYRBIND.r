...
EASYRBIND.read <- function(object) {
	...
	if(!object@blnUseFastRead) {
		...
		object@tblGWA 	<- tryCatch(
			...
			# fread(strFread,header=blnHeader, na.strings = object@astrMissing, stringsAsFactors=FALSE, colClasses = object@aClassesRead, skip=object@numRowSkip, select=aSelect, tmpdir=object@pathTmp),
			error = function(err) {
				strError = err$message
				val=strsplit(strError,"'",fixed=T)[[1]][length(strsplit(strError,"'",fixed=T)[[1]])]
				g=scan(file = object@fileIn, what=character(0), n = -1,sep = "\n",quiet=TRUE)
	...
	if(!object@blnHeader) object@tblGWA <- setNames(object@tblGWA, object@aHeader)
