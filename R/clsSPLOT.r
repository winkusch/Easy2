...
validSPLOT <- function(objSPLOT) {
	...
	if(objSPLOT@anumLwd[1] != -1) {
		for(i in 1:length(objSPLOT@anumLwd)) {
			
			numLwd = objSPLOT@anumLwd[i]
			if(numLwd <= 0)
				stop(paste("EASY ERROR:SPLOT\n Wrong anumLwd defined. PLease use positive numeric value.", sep=""))			
		}
	}
	
	if(!(objSPLOT@strFormat %in% c("png","pdf","tiff","jpeg")))
...
SPLOT.run <- function(objSPLOT, objGWA) {
	...
	numDefaultLwd			=	objSPLOT@numDefaultLwd
	...
	arcdLwdCrit				=	objSPLOT@arcdLwdCrit
	anumLwd					=	objSPLOT@anumLwd
