...
#Down
	...
	prototype = prototype(
						...
						numHeight				=	1200,	# pixel px # needs change for pdf!
...
setMethod("setMIAMIPLOT", signature = (object = "MIAMIPLOT"), function(object) {
	...
	aEqcSlotNamesIn = c("colInChr","colInPos",
						...
						"blnPlotVerticalLines", "numYAxisLim")
...
MIAMIPLOT.GWADATA.valid <- function(objMIAMIPLOT, objGWA) {
	...
	if(!(objMIAMIPLOT@strFormat %in% c("png","pdf","tiff","jpeg")))
	...
	if(objMIAMIPLOT@fileAnnot != "") {
		...
		### Cols exist?
			tblAnnot10<-read.table(objMIAMIPLOT@fileAnnot,header=T, sep="\t",  stringsAsFactors=FALSE, nrows = 10,comment.char="",quote="")
	...
	if(objMIAMIPLOT@numYAxisLim!=Inf & objMIAMIPLOT@blnYAxisBreak) stop(paste(" EASY ERROR:MIAMIPLOT\n You cannot set --numYAxisLim with --blnYAxisBreak 1. PLease remove one parameter.", sep=""))	
...
MIAMIPLOT.run <- function(objMIAMIPLOT, objGWA) {
	...
	numYAxisLim			= 	objMIAMIPLOT@numYAxisLim
	...
	if(blnLogPval) {
		...
		if(any(isRemove)) {
			objGWA@tblGWA[[iValUp]][which(isRemove)] <- NA
		}
		##
		...
		isRemove = isBelowOffset | isOutRange | isInf
		if(any(isRemove)) {
			objGWA@tblGWA[[iValDown]][which(isRemove)] <- NA
		}
	} else {
	...
	tblPlot <- setNames(tblPlot, c("yUp","yDown","chr","pos"))
	...
	## 211004 resort to have highlights on top
	# astrDefaultColourChrUp
	idxDefCol = as.integer(tblPlot$colplotUp %in% astrDefaultColourChrUp)
	tblPlot = tblPlot[order(idxDefCol, decreasing = TRUE),]
	
	if(numYAxisLim != Inf) arr_ylim = c(-numYAxisLim,numYAxisLim)
	...
	idxDefCol = as.integer(tblPlot$colplotDown %in% astrDefaultColourChrDown)
	tblPlot = tblPlot[order(idxDefCol, decreasing = TRUE),]
	...
	# prevent Chr labels being dropped: 
